...
class PDFPSReporte:
    ...
    def __init__(self, path,file_name,df,vdet):
        self.path = path
    ...
    def firstPage(self):
        ...
        psDetalle = ParagraphStyle('Resumen', fontSize=20, leading=25, justifyBreaks=1, alignment=TA_CENTER, justifyLastLine=1)
        text = """Vul AI<br/>
        Vulnerability Detection Report <br/>
        File Name: {file_name}<br/>
        Date: {date}<br/>        
        """.format(file_name=self.filename,date=datetime.now())

        spacer = Spacer(50, 100)
        
        psDetalle1 = ParagraphStyle('Resumen', fontSize=15, leading=20, justifyBreaks=1, alignment=TA_LEFT, justifyLastLine=1)
        text1 = """Our System has analyzed the file {file_name} and found total of {total_func} functions in the file  <br/>
        Each function is analyzed by our fined tuned model to detect for possible vulnerabilities <br/>
        A Final report has been generated covering all functions<br/>
        <br/>        
        """.format(file_name=self.filename,total_func = len(self.df))
        
        paragraphReportSummary = Paragraph(text, psDetalle)
        paragraphReportSummary1 = Paragraph(text1, psDetalle1)
        self.elements.append(paragraphReportSummary)
        self.elements.append(spacer)
        self.elements.append(paragraphReportSummary1)
    ...
    def remoteSessionTableMaker(self):        
        ...
        text = 'TOTAL FUNCTIONS IN FILE {total_func}'.format(total_func=len(self.df))
        paragraphReportHeader = Paragraph(text, psHeaderText)
        self.elements.append(paragraphReportHeader)
        ...
        #print(data)
        table = Table(data, colWidths=[30, 160, 230, 150, 50])
    ...
    def inSiteSessionTableMaker(self):
        self.elements.append(PageBreak())
        psHeaderText = ParagraphStyle('Hed0', fontSize=12, alignment=TA_LEFT, borderWidth=3, textColor=self.colorOhkaBlue0)
        text = 'Vulnerability Description And Effects on Robotic Systems'
        paragraphReportHeader = Paragraph(text, psHeaderText)
        self.elements.append(paragraphReportHeader)
        ...
        """
        d = []
        textData = [ "CWE ID", "How it can effect robotic systems"]
        ...
        # """
        # d = []
        # textData = ["No.", "CWE ID", "How it can effect robotic systems"]
        ...
        # # Row for total
        # totalRow = ["Total CWE IDS ", "", "", "", len(self.vdet)]
        # for item in totalRow:
        #     ptext = "<font size='%s'>%s</font>" % (fontSize-1, item)
        #     p = Paragraph(ptext, alignStyle[1])
        #     formattedLineData.append(p)
        ...
        # #print(data)
        # table = Table(data, colWidths=[150, 500])
        # tStyle = TableStyle([ #('GRID',(0, 0), (-1, -1), 0.5, grey),
        #         ('ALIGN', (0, 0), (0, -1), 'LEFT'),
        #         #('VALIGN', (0, 0), (-1, -1), 'TOP'),
        #         ("ALIGN", (1, 0), (1, -1), 'RIGHT'),
...
if __name__ == '__main__':
    report = PDFPSReporte('psreport.pdf',"file_name","df")